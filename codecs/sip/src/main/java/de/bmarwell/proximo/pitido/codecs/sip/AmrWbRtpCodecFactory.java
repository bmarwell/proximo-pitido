...
public class AmrWbRtpCodecFactory extends NativeRtpCodecFactory {
    ...
    final AmrWbEncodeService encodeService;
     ...
     */
    @Inject
    public AmrWbRtpCodecFactory(AmrWbEncodeService encodeService) {
     ...
     * The proxy subclass generated by Weld calls this constructor; it never executes
     * application logic, so {@code encodeService} is left {@code null}.
     */
    AmrWbRtpCodecFactory() {
        probeLibrary("libvo-amrwbenc.so.0", "AMR-WB RTP codec");
        this.encodeService = null;
    }

    /**
     * Test constructor — provides an {@link AmrWbEncodeService} backed by the given executor.
     *
     * <p>Intended for unit tests that need a working codec without a CDI container.
     * The caller is responsible for the executor's lifecycle.
     * Test code is exempt from the forbidden-API check that prohibits unmanaged threading
     * in production sources.
     *
     * @param testExecutorService a single-thread executor (e.g. from
     *                            {@code Executors.newSingleThreadExecutor()}) for test use
     */
    AmrWbRtpCodecFactory(ExecutorService testExecutorService) {
        probeLibrary("libvo-amrwbenc.so.0", "AMR-WB RTP codec");
        this.encodeService = new AmrWbEncodeService(testExecutorService);
    }
    ...
    public AmrWbRtpCodec forCall(String offeredFmtp) {
        ...
        return new AmrWbRtpCodec(offeredFmtp, this.encodeService);
    }
